# 用户指令记忆

本文件记录了用户的指令、偏好和教导，用于在未来的交互中提供参考。

## 格式

### 用户指令条目
用户指令条目应遵循以下格式：

[用户指令摘要]
- Date: [YYYY-MM-DD]
- Context: [提及的场景或时间]
- Instructions:
  - [用户教导或指示的内容，逐行描述]

### 项目知识条目
Agent 在任务执行过程中发现的条目应遵循以下格式：

[项目知识摘要]
- Date: [YYYY-MM-DD]
- Context: Agent 在执行 [具体任务描述] 时发现
- Category: [代码结构|代码模式|代码生成|构建方法|测试方法|依赖关系|环境配置]
- Instructions:
  - [具体的知识点，逐行描述]

## 去重策略
- 添加新条目前，检查是否存在相似或相同的指令
- 若发现重复，跳过新条目或与已有条目合并
- 合并时，更新上下文或日期信息
- 这有助于避免冗余条目，保持记忆文件整洁

## 条目

[连续执行偏好]
- Date: 2026-04-29
- Context: 用户在拆分架构任务中明确说明
- Instructions:
  - 当用户要求连续拆分或连续实施时，中途不需要反复确认；应持续执行到结构清晰、无需继续拆分再统一汇报。

[术语定义-回合]
- Date: 2026-04-29
- Context: 用户在玩法规则讨论中明确纠正
- Instructions:
  - 在该项目中，“一场对局”定义为一个回合（round）。
  - 术语表达时应区分“对局/回合（round）”与“局内行动轮次（turn）”，避免混用。

[混乱棋局项目架构与运行方式]
- Date: 2026-04-29
- Context: Agent 在执行项目理解与代码浏览时发现
- Category: 代码结构|构建方法|代码模式
- Instructions:
  - 项目是“混乱棋局 / Chaos Chess”，核心玩法是将中国象棋、围棋、国际象棋与扑克效果融合到同一 9x10 棋盘中。
  - 根目录是当前可运行的 Vite 前端入口，`package.json` 提供 `npm run dev`、`npm run build`、`npm run preview`，默认端口由 `FRONTEND_PORT` 控制，`Makefile` 默认使用 Bun 启动。
  - 前端核心代码位于 `client/src/`，其中 `main.js` 当前承载主要游戏流程，`core/` 提供插件管理、棋盘、状态效果，`plugins/` 提供棋子和扑克插件。
  - Canvas 渲染职责已开始收敛到 `client/src/ui/Renderer.js`，`main.js` 应优先保留应用编排、输入处理和玩法流程。
  - 扑克手牌、出牌和扑克技能效果流程已拆到 `client/src/game/PokerController.js`，通知和击杀提示已拆到 `client/src/ui/FeedbackController.js`。
  - 回合推进、围棋落子提子、象棋选子移动与升变分别拆到 `client/src/game/TurnController.js`、`client/src/game/GoController.js`、`client/src/game/ChessController.js`。
  - 状态栏按钮、模式文案和插件列表展示已拆到 `client/src/ui/AppUIController.js`，`main.js` 继续收敛为应用编排层。
  - 棋盘点击输入与模式分发已拆到 `client/src/game/InputController.js`，减少 `main.js` 对坐标与事件细节的耦合。
  - 服务端代码位于 `server/src/`，使用 `ws` 提供 WebSocket 房间与回合广播能力，但当前客户端主流程仍以本地单机 Canvas 交互为主。
  - 围棋插件的关键设计是跨插件“气”系统：所有同阵营棋子不区分 `pluginSource`，相邻时共享气，并可被围棋提子机制移除。

[扑克效果售卖粒度]
- Date: 2026-04-29
- Context: 用户在经济系统规则澄清中明确说明
- Instructions:
  - 德州扑克应整体作为一个可购买效果，不要按牌型拆分成多个单独售卖项。

[德州扑克出牌按钮规则]
- Date: 2026-04-29
- Context: 用户在商店与出牌流程规则澄清中明确说明
- Instructions:
  - 只有购买“德州扑克”后，才显示或启用出牌按钮。
  - 购买德州扑克仅代表获得出牌权限，具体触发效果仍由实际打出的牌型决定。

[联机玩家凭证规则]
- Date: 2026-04-29
- Context: 用户在联机房间流程定义中新增约束
- Instructions:
  - 当前阶段玩家身份先仅以 IP 地址作为区分不同玩家的凭证。

[联机稳定性推进方式]
- Date: 2026-04-30
- Context: 用户在联机优化阶段明确要求
- Instructions:
  - 联机稳定性应先做详细步骤拆解，再按里程碑顺序实施，避免来回改动。
  - 当前优先推进里程碑 A：先冻结协议与状态同步字段，再落地幂等和乱序保护。

[翻转棋与走棋次数规则约束]
- Date: 2026-05-06
- Context: 用户在新增棋类需求中明确规则
- Instructions:
  - 新增棋类“翻转棋”：商店购买后每次获得 +2 枚翻转棋，且可多次购买。
  - 翻转棋“+2 枚”按每 2 枚占用 1 个商店特效格子计数，不是固定“=2 且仅一次”。
  - 翻转棋占有权规则为“夹击翻转”：仅翻转棋可触发，且闭合线两端必须都是我方翻转棋；一端翻转棋一端其他棋子不生效。
  - 落翻转棋视为一步走棋。
  - 每个特效都必须声明“视为一步走棋”或“不视为一步走棋”，后续要支持增减走棋次数类效果。
  - 每轮红黑默认只有 1 步走棋；失去走棋次数后立即切换到下一回合。
  - 翻转棋若获得异常状态（如冻结）应立即被提走。

[走棋次数强约束与落子选择]
- Date: 2026-05-06
- Context: 用户在翻转棋后续扩展规则中明确要求
- Instructions:
  - “走棋次数”需要抽象成统一约束，便于后续实现“额外走棋次数”与“剥夺对方走棋次数”等效果。
  - 落子模式下必须允许玩家主动选择放置“围棋”还是“翻转棋”，不能只按库存自动优先。

[骷髅复苏技能规则]
- Date: 2026-05-06
- Context: 用户在墓地机制后续扩展中新增规则
- Instructions:
  - 购买“骷髅复苏”后，本 round 允许使用骷髅复苏技能。
  - 使用骷髅复苏视为落子行为，消耗 1 点墓地资源。
  - 骷髅部署位置限制为己方半场任意空位。
  - 部署出的骷髅类型为兵/卒。
  - 部署骷髅视为一步走棋（需要消耗走棋次数）。

[以太步道具规则]
- Date: 2026-05-06
- Context: 用户在道具扩展中新增规则
- Instructions:
  - 新增道具“以太步”。
  - 使用时先选择一个己方棋子（任意棋子），再选择另一个己方棋子作为锚点。
  - 被选中的棋子可以移动到锚点周围 8 格中的任意空位。
  - 以太步视为一步走棋。
  - 以太步不允许吃子。

[烟雾弹道具规则]
- Date: 2026-05-06
- Context: 用户在道具扩展中新增规则
- Instructions:
  - 新增道具“烟雾弹”。
  - 烟雾弹效果实现参考现有扑克效果中的烟雾弹逻辑。

[道具次数与被动生效规则]
- Date: 2026-05-06
- Context: 用户对道具系统给出统一约束
- Instructions:
  - 除非有特殊说明，道具使用次数默认只有 1 次。
  - 国际象棋、骷髅复苏、五子棋这类被动道具，购买后即整局生效，不按主动次数消耗。

[视觉参考稿输出格式约束]
- Date: 2026-05-09
- Context: 用户要求重写视觉参考稿 v3
- Instructions:
  - 视觉参考稿中不要描述风格。
  - 只需要描述每个界面应包含的元素和按钮。

[基础棋盘定义]
- Date: 2026-05-09
- Context: 用户在视觉与界面约束中明确说明
- Instructions:
  - 基础棋盘必须是象棋棋盘。

[Lobby分页与跳转规则]
- Date: 2026-05-09
- Context: 用户要求按页面逐步实施并先完成 lobby
- Instructions:
  - 页面按一页一页实现，先做 lobby。
  - lobby 页面只包含“加入房间”和“创建房间”两个入口。
  - 点击“加入房间”后跳转到房间大厅页面。
  - 点击“创建房间”后跳转到房间内页面。
  - 输入房间码的位置放在房间大厅页面，不放在 lobby 页面。

[Lobby规则与设置入口]
- Date: 2026-05-09
- Context: 用户新增 lobby 底部入口要求
- Instructions:
  - image-12 作为“规则”按钮，放在创建房间按钮左下角。
  - image-13 作为“设置”按钮，放在创建房间按钮右下角。
  - 点击“规则”跳转到游戏帮助页面（当前先留空）。
  - 点击“设置”跳转到设置页面（当前先留空）。

[五子棋判定范围]
- Date: 2026-05-11
- Context: 用户在规则修正中明确说明
- Instructions:
  - 五子棋模式下“五子连珠”仅计算围棋子（Go 插件棋子）。
  - 其他棋类棋子不计入五子连珠判定。

[决心道具规则]
- Date: 2026-05-11
- Context: 用户在新增特殊道具需求中明确说明
- Instructions:
  - 新增特殊道具“决心”，整场 BO3/BO5 每方仅可购买一次。
  - 决心是主动道具，使用不视为一步走棋。
  - 第一次使用时保存当前棋盘状态、双方经济、场地资源、技能次数为存档。
  - 第二次使用时可在本局任意时点读档，包括对方回合。
  - 读档后恢复到存档状态，并切换到对方回合。
  - 决心不能回溯整场 BO3/BO5 仅可使用一次的技能状态。

[联机槽位显示规则]
- Date: 2026-05-11
- Context: 用户在决心道具联机显示规则中明确说明
- Instructions:
  - 联机对战中，红黑双方的道具槽位都应显示。
  - 以上红下黑为例，棋盘上方显示红方 3 个槽位，下方显示黑方 3 个槽位。
  - 对手的道具槽位只能展示，不能点击使用。
